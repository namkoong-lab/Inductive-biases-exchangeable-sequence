...
from data.load_data import (
    load_active_learning_add_data,
    load_active_learning_data,
    load_active_learning_matmul_data,
    load_active_learning_region_data,
    load_classical_contextual_bandit_data,
    load_contextual_bandit_data,
    load_mixed_contextual_bandit_data,
    load_reward_data,
    load_toy_reward_data,
)
...
class Trainer:
    ...
    def set_up(self, training_args):
        ...
        if training_args.load_from_checkpoint:
            self.model.load_state_dict(
                torch.load(training_args.checkpoint_path, weights_only=True)
            )
        ...
        if self.data_args.dataset_name == "toy_data":
            self.training_dataloader, self.test_dataloader = load_toy_reward_data(
                self.training_args, self.data_args, self.model_args
            )
        elif self.data_args.dataset_name == "contextual_bandit":
            self.training_dataloader, self.test_dataloader = (
                load_contextual_bandit_data(
                    self.training_args, self.data_args, self.model_args
                )
            )
        ...
        elif self.data_args.dataset_name == "mixed_contextual_bandit":
            self.training_dataloader, self.test_dataloader = (
                load_mixed_contextual_bandit_data(
                    self.training_args, self.data_args, self.model_args
                )
            )
        elif self.data_args.dataset_name == "active_learning_matmul":
            self.training_dataloader, self.test_dataloader = (
                load_active_learning_matmul_data(
                    self.training_args, self.data_args, self.model_args
                )
            )
        elif self.data_args.dataset_name == "active_learning_add":
            self.training_dataloader, self.test_dataloader = (
                load_active_learning_add_data(
                    self.training_args, self.data_args, self.model_args
                )
            )
        elif self.data_args.dataset_name == "active_learning_region":
            self.training_dataloader, self.test_dataloader = (
                load_active_learning_region_data(
                    self.training_args,
                    self.data_args,
        ...
        elif self.data_args.dataset_name == "active_learning_region_ha":
            self.training_dataloader, self.test_dataloader = (
                load_active_learning_region_data(
                    self.training_args,
                    self.data_args,
                    self.model_args,
                    with_high_aleatoric=True,
                )
            )
        elif self.data_args.dataset_name.startswith("active_learning_hregions"):
            num_regions = self.data_args.dataset_name.split("_")[-1]
            self.training_dataloader, self.test_dataloader = (
                load_active_learning_region_data(
                    self.training_args,
                    self.data_args,
                    self.model_args,
                    with_high_regions=True,
                    num_regions=num_regions,
                )
            )
        elif self.data_args.dataset_name.startswith("al"):
            num_regions = self.data_args.dataset_name.split("_")[-1]
            self.training_dataloader, self.test_dataloader = load_active_learning_data(
                self.training_args,
                self.data_args,
                self.model_args,
                al_data_name=self.data_args.dataset_name,
            )
        else:
            self.training_dataloader, self.test_dataloader = load_reward_data(
                self.training_args, self.data_args, self.model_args
            )
        ...
        self.accelerator.print(
            f"[MODEL PARAMS] Total number of parameters: {total_params}"
        )
    ...
    def evaluate(
        self, data_loaders, initial=False, final=False, iter=None, prefix=None
    ):
        if initial:
        ...
        eval_metric_list, eval_metric_mean = self.eval_metric_func(
            dataloader, self.accelerator, self.model, self.training_args.test_horizon
        )
        if self.accelerator.is_main_process:
            for metric, values in eval_metric_list.items():
                ...
                line_series = wandb.plot.line(
                    table, "time_step", "value", title=f"{prefix} {metric} T{iter}"
                )
        ...
        if accelerator.is_main_process:
            for metric in eval_metric_list:
                eval_metric_list[metric] = (
                    (eval_metric_list[metric].sum(0) / n.sum()).detach().cpu().numpy()
                )
            for metric in eval_metric_mean:
                eval_metric_mean[metric] = (
                    (eval_metric_mean[metric].sum(0) / n.sum()).detach().cpu().numpy()
                )
    ...
    def save_model(self, checkpoint_iter=None):
        ...
        if self.accelerator.is_local_main_process:
            if self.data_args.dataset_name == "contextual_bandit":
                saving_dir = f"/shared/share_mala/tyen/seqb/CB_{self.model_args.dim_llm_embedding}/UQ-{self.model_args.uncertainty}-Loss-{self.model_args.loss_type}-Horizon-{self.training_args.train_horizon}-grad-{self.model_args.gradient_type}_Noise_{self.data_args.noise_scale}"
                ...
                if checkpoint_iter == None:
                    ...
                    torch.save(
                        ...
                        f"{saving_dir}/model_checkpoint_{checkpoint_iter}.pt",
                    )

            elif self.data_args.dataset_name == "classical_contextual_bandit":
                saving_dir = f"/shared/share_mala/tyen/seqb/CCB_{self.model_args.dim_llm_embedding}/Model-{self.model_args.model_type}-Horizon-{self.training_args.train_horizon}_Noise_{self.data_args.noise_scale}"
                if not os.path.exists(saving_dir):
                    os.makedirs(saving_dir)
                if checkpoint_iter == None:
                    torch.save(unwrapped_model.state_dict(), f"{saving_dir}/model.pt")
            ...
            elif self.data_args.dataset_name == "active_learning":
                saving_dir = f"/shared/share_mala/tyen/seqb/AL_{self.model_args.dim_llm_embedding}/Model-{self.model_args.model_type}-Horizon-{self.training_args.train_horizon}_Noise_{self.data_args.noise_scale}"
                if not os.path.exists(saving_dir):
                    os.makedirs(saving_dir)
                if checkpoint_iter == None:
                    torch.save(unwrapped_model.state_dict(), f"{saving_dir}/model.pt")
                ...
                saving_dir = f"/shared/share_mala/tyen/seqb/dataset={self.data_args.dataset_name}_{self.model_args.dim_llm_embedding}/Model-{self.model_args.model_type}-Horizon-{self.training_args.train_horizon}_Noise_{self.data_args.noise_scale}"
                if not os.path.exists(saving_dir):
                    os.makedirs(saving_dir)
                if checkpoint_iter == None:
                    torch.save(unwrapped_model.state_dict(), f"{saving_dir}/model.pt")
