...
def run_exp(process_id):
    ...
    autoreg_checkpoint_dir = '/shared/share_mala/Leon/CCB_1/Model-autoreg-Horizon-200_Noise_0.1/model_checkpoint_400.pt'

    excg_config_path = '../scripts/uq_normal_excg.yaml'
    excg_checkpoint_dir = '/shared/share_mala/Leon/CCB_1/Model-excg-Horizon-200_Noise_0.1/model_checkpoint_400.pt'
    ...
    # Apply different scaling factors for each arm from the specified list
    scaling_factors = torch.tensor([random.choice([1.0, 1.5, 2.0, 2.5, 3.0]) for _ in range(num_arms)])
    ...
    arm_noise_scales = torch.linspace(0.1, 1.0, num_arms)[torch.randperm(num_arms)]  # Creates evenly spaced noise scales from 0.1 to 1.0 and permutes them
