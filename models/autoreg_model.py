...
from models.modules import build_mlp, RiemannDistribution
...
class Autoreg_Model(nn.Module):
    ...
    def construct_causal_mask(self, n):
        '''
    ...
    def forward(self, batch):
        ...
        mask = self.construct_causal_mask(x.shape[1])
        ...
        if self.uncertainty == 'normal':
            ...
            if self.loss_type == 'logprob':
                ...
                # self.accelerator.print(f'nll: {nll.shape}')
                loss = nll.sum(-1).mean()
    ...
    def construct_causal_prediction_mask(self, batch):
    ...
    def predict(self, batch):
        ...
        mask = self.construct_causal_prediction_mask(batch)
        mask = mask.to(embeddings.dtype)
        mask = mask.to(device)
